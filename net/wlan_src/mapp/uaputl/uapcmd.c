            ...
            if (cmd_buf->domain.country_code[0] ||
                ...
                if (cmd_buf->domain.country_code[2] == 'I') {
                    if ((channel != 36) && (channel != 40) && (channel != 44) &&
                        (channel != 48)) {
                        printf
                            ("ERR: Channel not valid for indoor operation!\n");
                        ret = UAP_FAILURE;
                        goto done;
                    }
                }
...
apcmd_sys_cfg_channel(int argc, char *argv[])
{
    ...
    if (argc && (atoi(argv[0]) > MAX_CHANNELS_BG))
        band = BAND_A;
    ...
    if ((argc == 1) || ((argc == 2) && !(atoi(argv[1]) & BITMAP_ACS_MODE))) {
        ...
        if (state_80211d) {
            ret = check_channel_validity_11d(atoi(argv[0]), band, 1);
            if (ret == UAP_FAILURE) {
                print_sys_cfg_channel_usage();
                return ret;
            }
...
apcmd_sys_cfg_channel_ext(int argc, char *argv[])
{
    ...
    if (argc > 1) {
        ...
        if ((argc == 1) && (atoi(argv[0]) > MAX_CHANNELS_BG))
            band = BAND_A;
    }
    /* Check if channel is valid for given domain and set domain info. */
    if ((argc == 1) || (argc == 2) ||
    ...
    if (argc) {
        ...
        if (state) {
            for (i = 0; i < argc; i++) {
                ...
                ret = check_channel_validity_11d(chan_number, band, 0);
                if (ret == UAP_FAILURE) {
                    print_sys_cfg_channel_usage();
                    return UAP_FAILURE;
                }
        ...
        if (atoi(argv[0]) == 0) {
            ...
            if (0 == get_fw_info(&fw)) {
                if ((fw.hw_dev_mcs_support & 0x0f) >= 2)
                    tlv->ht_cap.supported_mcs_set[1] = DEFAULT_MCS_SET_1;
            }
            if (argc >= 2) {
