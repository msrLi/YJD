...
mutex_t AddRemoveCardSem;
/**
 * the maximum number of adapter supported 
 **/
#define MAX_MLAN_ADAPTER    1
/**
 ...
 *  @return 	   MLAN_STATUS_SUCCESS or MLAN_STATUS_FAILURE
 */
extern mlan_callbacks woal_callbacks;
...
woal_init_sw(moal_handle * handle)
{
	...
	device = (mlan_device*)mem_calloc(sizeof(mlan_device), 1);
 ...
 */
static mlan_status
woal_request_fw_dpc(moal_handle * handle, struct firmware *firmware)
{
    mlan_status ret = MLAN_STATUS_SUCCESS;
    ...
    if ((ret = woal_init_fw_dpc(handle)) != 0)
        goto done;
    if ((ret = woal_add_card_dpc(handle)) != 0)
        goto done;
...
/*
static void
woal_request_fw_callback(struct firmware *firmware, void *context)
{
    ENTER();
...
#endif
#ifdef FW_OUTSIDE
	handle->firmware.data = (u8*)0;
	handle->firmware.size = 249580;
#else
	handle->firmware.data = (u8*)fm_88w8287;
	handle->firmware.size = sizeof(fm_88w8287);
#endif
	ret = woal_request_fw_dpc(handle, &handle->firmware);
                      ...
                      mlan_buffer * pmbuf, struct sk_buff *skb)
{
    ...
    case __constant_htons(ETH_P_IP):
        tid = /*(IPTOS_PREC(SKB_TOS(skb)) >> IPTOS_OFFSET)*/0;
		//p_err_miss;
...
#define MAX_WMM_QUEUE   4
    ...
    if ((dev = mem_calloc(sizeof(moal_private) + sizeof(struct net_device), 1)) == 0) {
#else
...
#ifdef REASSOCIATION
    handle->reassoc_on |= MBIT(priv->bss_index); //ADD BY LCK
#endif
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,29)
                  ...
                  t_u32 src_ip, t_u16 src_port, t_u32 dst_ip, t_u16 dst_port)
{
    ...
    list_for_each_entry(struct tcp_sess, tcp_sess, &priv->tcp_sess_queue, link) {
...
#define TCP_ACK_RECV_WARMING_UP  1000   /* recovery */
    ...
    if (tcph->syn & tcph->ack) {
        ...
        if ((tcp_sess = kmalloc(sizeof(struct tcp_sess), GFP_ATOMIC)) == 0) {
		...
		uint16_t source = tcph->source;
		uint16_t dest = tcph->dest;
        tcp_sess = woal_get_tcp_sess(priv, saddr, source,
                                     daddr, dest);
        if (tcp_sess == NULL) {
            return 0;
        }
...
woal_hard_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
    ...
    if (/*priv->enable_tcp_ack_enh == MTRUE*/0) {
...
woal_alloc_mlan_buffer(moal_handle * handle, int size)
{
    ...
    ENTER();
    if ((pmbuf = kmalloc(sizeof(mlan_buffer), GFP_ATOMIC)) == 0) {
        PRINTM(MERROR, "%s: Fail to alloc mlan buffer\n", __FUNCTION__);
        LEAVE();
        return NULL;
    }
    memset((t_u8 *) pmbuf, 0, sizeof(mlan_buffer));
    if ((skb = dev_alloc_skb(size)) == 0) {
...
woal_alloc_mlan_ioctl_req(int size)
{
    ...
    if ((req =
         ...
         kmalloc((sizeof(mlan_ioctl_req) + size + sizeof(int) +
                  sizeof(wait_queue)), GFP_ATOMIC)) == 0) {
...
woal_reassociation_thread(void *data)
{
    ...
    for (;;) {
        ...
        wait_event(pmoal_thread->wait_q);
        ...
        for (i = 0; i < handle->priv_num && (priv = handle->priv[i]); i++) {
			...
			key = (char*)mem_calloc(MLAN_MAX_PASSPHRASE_LENGTH + 1, 1);
			essid = (char*)mem_calloc(MLAN_MAX_SSID_LENGTH + 1, 1);
			if(key && essid)
			{
				memcpy(key, priv->prev_passphrase.passphrase, MLAN_MAX_PASSPHRASE_LENGTH);
				key[MLAN_MAX_PASSPHRASE_LENGTH] = 0;
				memcpy(essid, priv->prev_ssid_bssid.ssid.ssid, MLAN_MAX_SSID_LENGTH);
				essid[MLAN_MAX_SSID_LENGTH] = 0;
    ...
    if (IS_STA_CFG80211(cfg80211_wext)) {
        if (!priv->cfg_disconnect &&
            ...
            /* This function must be called only when disconnect issued by the
               ...
               valid */
            //g80211_disconnected(priv->netdev,
            //                    WLAN_REASON_DEAUTH_LEAVING, NULL, 0,
            //                    GFP_KERNEL);
            p_err_miss;
        }
        priv->cfg_disconnect = FALSE;
    }
#endif /* STA_CFG80211 */

    memset(event_buf, 0, sizeof(event_buf));
    custom_len = strlen(CUS_EVT_AP_CONNECTED);
    strncpy((char*)event_buf, CUS_EVT_AP_CONNECTED, custom_len);
...
#endif
again:
	if(((struct sdio_mmc_card *) handle->card)->func->sdio_int_pending)
	{
		redo_once= 0;
		((struct sdio_mmc_card *) handle->card)->func->sdio_int_pending = FALSE;
	...
	if(redo_once == 0 && ((struct sdio_mmc_card *) handle->card)->func->sdio_int_pending == FALSE)
	{
		redo_once = 1;
		if(read_int_status(handle))
		{
			((struct sdio_mmc_card *) handle->card)->func->sdio_int_pending = TRUE;
			goto again;
		}
	}
...
woal_add_card(void *card)
{
    ...
    if ((handle = kmalloc(sizeof(moal_handle), GFP_KERNEL)) == 0) {
...
//    }
...
#else
    handle->workqueue = create_workqueue("MOAL_WORK_QUEUE", MOAL_WROK_STACK_SIZE, TASK_MOAL_WROK_PRIO);
#endif
    if (!handle->workqueue)
        goto err_kmalloc;
    ...
    thread_create((void (*)(void *))woal_reassociation_thread, 
			&handle->reassoc_thread, 
			REASSOCIATION_THREAD_PRIO, 
			0, 
			REASSOCIATION_THREAD_STACK_SIZE, 
 ...
 */
int
woal_init_module(struct sdio_func *func)
 {
	...
	ret = woal_sdio_probe(func);
	if(ret)
	{
		PRINTM(MFATAL, "woal_sdio_probe Failed\n");
	}
    LEAVE();
    return ret;
}
