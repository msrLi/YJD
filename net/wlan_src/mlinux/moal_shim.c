            ...
            IN t_u32 size, IN t_u32 flag, OUT t_u8 ** ppbuf)
{
    ...
    if ((*ppbuf = kmalloc(size, mem_flag)) == 0) {
                ...
                IN t_void(*callback) (t_void * pcontext), IN t_void * pcontext)
{
    ...
    if ((timer =
         (moal_drv_timer *) kmalloc(sizeof(moal_drv_timer), GFP_KERNEL)) == 0)
        return MLAN_STATUS_FAILURE;
...
moal_free_timer(IN t_void * pmoal_handle, IN t_void * ptimer)
{
    ...
    if (timer) {
        if ((timer->timer_is_canceled == MFALSE) && timer->time_period) {
			...
			woal_free_timer(timer); //add by lck
        }
        kfree(timer);
    }
 ...
 *  @return    		MLAN_STATUS_SUCCESS or MLAN_STATUS_FAILURE 
 */
#define PRE_ALLOC_MOAL_LOCK_NUM 40
static moal_lock g_moal_lock[PRE_ALLOC_MOAL_LOCK_NUM];
static uint8_t g_moal_lock_cnt = 0;
...
moal_init_lock(IN t_void * pmoal_handle, OUT t_void ** pplock)
{
	...
	if(g_moal_lock_cnt < PRE_ALLOC_MOAL_LOCK_NUM)
	{
		mlock = &g_moal_lock[g_moal_lock_cnt];
		g_moal_lock_cnt++;
	}else{
    	if ((mlock = (moal_lock *) kmalloc(sizeof(moal_lock), GFP_ATOMIC)) == 0)
        	return MLAN_STATUS_FAILURE;
	}
...
moal_free_lock(IN t_void * pmoal_handle, IN t_void * plock)
{
    ...
    if (mlock) {
		if(mlock >= &g_moal_lock[0] && mlock <= &g_moal_lock[PRE_ALLOC_MOAL_LOCK_NUM - 1])
			;
		else
        	kfree(mlock);
                 ...
                 IN t_u32 offset, IN t_u32 len, OUT t_u8 * pbuf)
{
	...
	if(handle->firmware.data)
    	memcpy(pbuf, handle->firmware.data + offset, len);
	else{
		if(read_firmware(offset, len, pbuf))
			return MLAN_STATUS_FAILURE;
	}
    ...
    if (pmbuf) {
        ...
        if (priv) {
            if (skb) {
                ...
                if ((skb = dev_alloc_skb(pmbuf->data_len + MLAN_NET_IP_ALIGN)) == 0) {
...
extern void cfg80211_cqm_rssi_notify(struct net_device *dev,
...
moal_recv_event(IN t_void * pmoal_handle, IN pmlan_event pmevent)
{
    ...
    case MLAN_EVENT_ID_DRV_CONNECTED:
        ...
        memmove(pmevent->event_buf + custom_len, pmevent->event_buf,
                pmevent->event_len);
        strncpy((char*)pmevent->event_buf, CUS_EVT_AP_CONNECTED, custom_len);
...
//        }
...
#ifdef UAP_SUPPORT
        if ((pmpriv =
             woal_get_priv((moal_handle *) pmoal_handle, MLAN_BSS_ROLE_UAP)) != 0) {
            pmevent->event_id = MLAN_EVENT_ID_FW_HS_WAKEUP;
    ...
    case MLAN_EVENT_ID_DRV_HS_ACTIVATED:
        ...
        if ((pmpriv =
             woal_get_priv((moal_handle *) pmoal_handle, MLAN_BSS_ROLE_UAP)) != 0) {
            pmevent->event_id = MLAN_EVENT_ID_DRV_HS_ACTIVATED;
            woal_broadcast_event(pmpriv, (t_u8 *) & pmevent->event_id,
                                 sizeof(t_u32));
        }
#endif
    ...
    case MLAN_EVENT_ID_DRV_HS_DEACTIVATED:
        ...
        if ((pmpriv =
             woal_get_priv((moal_handle *) pmoal_handle, MLAN_BSS_ROLE_UAP)) != 0) {
            pmevent->event_id = MLAN_EVENT_ID_DRV_HS_DEACTIVATED;
            woal_broadcast_event(pmpriv, (t_u8 *) & pmevent->event_id,
                                 sizeof(t_u32));
        }
#endif
