...
woal_timer_handler(unsigned long fcontext)
{
    ...
    if (timer->timer_is_periodic == MTRUE) {
        mod_timer(timer->tl, /*jiffies + ((timer->time_period * HZ) / 1000)*/timer->time_period);
    } else {
                      ...
                      void *FunctionContext)
{
    ...
    timer->tl = timer_setup(1000, 0, TimerFunction, FunctionContext);
...
woal_mod_timer(pmoal_drv_timer timer, t_u32 MillisecondPeriod)
{
    ...
    mod_timer(timer->tl, /*jiffies + (MillisecondPeriod * HZ) / 1000*/MillisecondPeriod);
 ...
 */
static inline void
woal_free_timer(moal_drv_timer * timer)
{
    timer_free(timer->tl);
	timer->tl = NULL;
    timer->timer_is_canceled = MTRUE;
...
#if /*LINUX_VERSION_CODE < KERNEL_VERSION(2,6,19)*/1
...
#define MOAL_INIT_SEMAPHORE(x)    	*(x) = mutex_init(__FUNCTION__)
/** Initialize semaphore */
...
#define MOAL_ACQ_SEMAPHORE_BLOCK(x)		mutex_lock(*(x))	//down_interruptible(x)
/** Acquire semaphore without blocking */
#define MOAL_ACQ_SEMAPHORE_NOBLOCK(x)	mutex_lock(*(x))	//down_trylock(x)
/** Release semaphore */
#define MOAL_REL_SEMAPHORE(x) 		mutex_unlock(*(x))	//up(x)
...
#define SKB_TOS(skb) (_ip_hdr(skb)->tos)
#else
...
struct _moal_private
{
	...
	mlan_passphrase_t prev_passphrase;
...
#define DBG_HEXDUMP_MCMND	DBG_HEXDUMP_MCMD_D
