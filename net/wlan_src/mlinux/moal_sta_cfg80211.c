...
struct cfg80211_ops woal_cfg80211_sta_ops = {
	/*.suspend = 0,
	.resume = 0,
	.set_wakeup = 0,
	.add_virtual_intf = 0,
                   ...
                   mlan_ssid_bssid * ssid_bssid, bool enable)
{
    ...
    req->action = MLAN_ACT_GET;
#if 0 //edit by lck 无法获取到passphrase??
    /* Try Get All */
...
woal_cfg80211_assoc(moal_private * priv, void *sme)
{
...
#if 1	//add by lck 根据搜到的ap信息填充完善conn_param结构
	...
	if (priv->media_connected == MTRUE) //add by lck
...
#ifdef REASSOCIATION	
	priv->prev_ssid_bssid = ssid_bssid; //add by lck
#endif	
