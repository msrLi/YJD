...
#else
    int mc_count = /*netdev_mc_count(dev)*/MLAN_MAX_MULTICAST_LIST_SIZE + 1; //这里简单处理，接收所有多播数据
#endif
...
woal_hostcmd_ioctl(struct net_device *dev, struct ifreq *req)
{
    ...
    if (copy_from_user(&buf_len, req->ifr_ifru.ifru_data, sizeof(t_u32))) {
        PRINTM(MERROR, "Copy from user failed\n");
        ret = -EFAULT;
        goto done;
    }
    ...
    /* get command header */
    if (copy_from_user
        (&cmd_header, (char*)req->ifr_ifru.ifru_data + sizeof(t_u32),
    ...
    /* get the whole command from user */
    if (copy_from_user
        (misc->param.hostcmd.cmd, 
		(char*)req->ifr_ifru.ifru_data + sizeof(t_u32),
         misc->param.hostcmd.len)) {
        PRINTM(MERROR, "copy from user failed\n");
        ret = -EFAULT;
        goto done;
    ...
    if (copy_to_user
        ((char*)req->ifr_ifru.ifru_data + sizeof(t_u32), (t_u8 *) misc->param.hostcmd.cmd,
         misc->param.hostcmd.len)) {
        ret = -EFAULT;
        goto done;
    }
...
woal_custom_ie_ioctl(struct net_device *dev, struct ifreq *req)
{
    ...
    if ((custom_ie = kmalloc(sizeof(mlan_ds_misc_custom_ie), GFP_KERNEL)) == 0) {
        ret = -ENOMEM;
        goto done;
    }
    ...
    if (copy_from_user
        (custom_ie, req->ifr_ifru.ifru_data, sizeof(mlan_ds_misc_custom_ie))) {
        PRINTM(MERROR, "Copy from user failed\n");
        ret = -EFAULT;
        goto done;
    }
    ...
    if (ioctl_req->action == MLAN_ACT_GET) {
        if (copy_to_user
            (req->ifr_ifru.ifru_data, &misc->param.cust_ie,
...
woal_send_host_packet(struct net_device *dev, struct ifreq *req)
{
    ...
    if (copy_from_user(&packet_len, req->ifr_ifru.ifru_data, sizeof(packet_len))) {
        PRINTM(MERROR, "Copy from user failed\n");
        ret = -EFAULT;
        goto done;
    }
...
woal_get_bss_type(struct net_device *dev, struct ifreq *req)
{
    ...
    if (copy_to_user(req->ifr_ifru.ifru_data, &bss_type, sizeof(int))) {
                  ...
                  t_u8 wait_option, t_u8 * bss_role)
{
    ...
    if (req)
        kfree(req);
    LEAVE();
    return (ret == 0)?MLAN_STATUS_SUCCESS:MLAN_STATUS_FAILURE;
}

#if /*defined(STA_WEXT) || defined(UAP_WEXT)*/1
/**
...
woal_cancel_scan(moal_private * priv, t_u8 wait_option)
{
    ...
    for (i = 0; i < handle->priv_num; i++) {
        if (IS_STA_CFG80211(cfg80211_wext) && handle->priv[i]->scan_request) {
            cfg80211_scan_done(handle->priv[i]->scan_request, TRUE);
...
{
    t_u8 *ptr = (t_u8 *)buf + strlen("BGSCAN-CONFIG") + 1;
    ...
    while (buf_left >= 2) {
        ...
        case WEXT_CSCAN_SSID_SECTION:
            ...
            if (ssid_len && (num_ssid < (MRVDRV_MAX_SSID_LIST_LENGTH - 1))) {
                strncpy((char*)priv->scan_cfg.ssid_list[num_ssid].ssid, (char*)ptr + 2,
                        ssid_len);
...
woal_set_combo_scan(moal_private * priv, char *buf, int length)
{
    ...
    t_u8 *ptr = (t_u8 *)buf + WEXT_CSCAN_HEADER_SIZE;
    ...
    while (buf_left >= 2) {
        ...
        case WEXT_CSCAN_SSID_SECTION:
            ...
            if (ssid_len && (num_ssid < (MRVDRV_MAX_SSID_LIST_LENGTH - 1))) {
                strncpy((char *)scan_cfg.ssid_list[num_ssid].ssid, (char *)ptr + 2, ssid_len);
...
{
    mlan_status ret = MLAN_STATUS_SUCCESS;
    t_u8 *ptr = (t_u8 *)buf + NL80211_SCANCFG_HEADER_SIZE;
