...
#define MRVDRV_MAX_CFP_CODE_BG          1//0
...
#define DEFAULT_SCAN_BEACON_BUFFER      1024 //4096
...
typedef struct _mlan_private
{
    ...
    t_u8 ewpa_query;
    /** Encryption Key*/
#if WPA_IE_ALLOC_IN_USE
	t_u8 *wpa_ie;
	t_u16 max_wpa_ie_len;
#else
    t_u8 wpa_ie[256];
#endif
    /** WPA IE length */
    ...
    /** WAPI IE */
#if WAPI_IE_ALLOC_IN_USE
	t_u8 *wapi_ie;
	t_u16 max_wapi_ie_len;
#else
    t_u8 wapi_ie[256];
#endif
    /** WAPI IE length */
...
#ifdef STA_SUPPORT
#if ASSOC_RSP_BUFF_ALLOC_IN_USE
	t_u8 *assoc_rsp_buf;
	t_u32 max_assoc_rsp_size;
#else
     ...
     */
#if GEN_IE_BUF_ALLOC_IN_USE
	t_u8 *gen_ie_buf;
	t_u8 max_gen_ie_buf_len;
#else
...
wlan_is_tx_pause(mlan_private * priv, t_u8 * ra)
{
    ...
    if ((sta_ptr = wlan_get_station_entry(priv, ra)) != 0) {
