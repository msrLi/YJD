                   ...
                   t_u32 cipher_suite)
{
    ...
    if (((pbss_desc->prsn_ie) &&
        ...
        if ((ret = search_oui_in_ie(pmadapter, ie_body, oui)) != 0) {
            LEAVE();
            return ret;
        }
    }
...
#if WPS_IE_ALLOC_IN_USE
	if(!pmpriv->wps.wps_ie || !pmpriv->wps.max_wps_ie_len)
		return;
                                ...
                                IN t_u8 ** pbeacon_info, IN t_u32 * bytes_left)
{
    ...
    *bytes_left -= beacon_size;
    ...
    memcpy(pmadapter, (void*)&fixed_ie.beacon_interval, (const void*)pcurrent_ptr, 2);
    pbss_entry->beacon_period = wlan_le16_to_cpu(fixed_ie.beacon_interval);
    pcurrent_ptr += 2;
    bytes_left_for_current_beacon -= 2;

    /* Capability information is 2 bytes long */
    memcpy(pmadapter, (void*)&fixed_ie.capabilities, (const void*)pcurrent_ptr, 2);
 ...
 *  @return           N/A
 */
DECLARE_MONITOR_ITEM("max_beacon_size", max_beacon_size); 
static t_void
                                  ...
                                  IN BSSDescriptor_t * pnew_beacon)
{
    ...
    if (pmadapter->pscan_table[beacon_idx].pbeacon_buf) {
        ...
        if (new_bcn_size == old_bcn_size) {
             ...
             */
        #if SYS_MONITOR
			if(GET_MONITOR_ITEM_VALUE(max_beacon_size) < new_bcn_size){
				
				SET_MONITOR_ITEM_VALUE(max_beacon_size,new_bcn_size);
			}
		#endif	
...
//说明:这个函数做了一点改造,为了节省内存,我们搜索到的ap
//都直接调用handle_user_scan_result给用户,而不保存到scan_table
//scan_table只能保存一个成员,用于wifi连接的时候使用
extern void handle_user_scan_result(BSSDescriptor_t *pscan_table); 
mlan_status
                     ...
                     IN HostCmd_DS_COMMAND * resp, IN t_void * pioctl_buf)
{
    ...
    for (idx = 0; idx < pscan_rsp->number_of_sets && bytes_left; idx++) {
        ...
        if (wlan_interpret_bss_desc_with_ie(pmadapter,
			...
			handle_user_scan_result(bss_new_entry);
    ...
    for (idx = 0; idx < number_of_sets && bytes_left >
        ...
        if (wlan_interpret_bss_desc_with_ie(pmadapter,
            ...
            if (pscan_info_tlv) {
                ...
                memcpy(pmpriv->adapter, &tsf_val, (const void*)&pscan_info_tlv->tsf,
                       sizeof(tsf_val));
                tsf_val = wlan_le64_to_cpu(tsf_val);
                memcpy(pmpriv->adapter, &bss_new_entry->network_tsf,
                       &tsf_val, sizeof(bss_new_entry->network_tsf));
