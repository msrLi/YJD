 ...
 *  @return 	   MLAN_STATUS_SUCCESS
 */
DECLARE_MONITOR_ITEM("event_body_size", event_body_size);
DECLARE_MONITOR_ITEM("data_pkt_cnt", data_pkt_cnt);
                      ...
                      t_u32 upld_typ)
{
    ...
    case MLAN_TYPE_DATA:
		...
		INC_MONITOR_ITEM_VALUE(data_pkt_cnt);
        break;
...
#if EVENT_BODY_ALLOC_IN_USE
			if(pmadapter->event_body_len < (pmadapter->upld_len - MLAN_EVENT_HEADER_LEN))
			{
				p_err("event_body_len too short:%d, need:%d", pmadapter->event_body_len, (pmadapter->upld_len - MLAN_EVENT_HEADER_LEN));
				mem_free(pmadapter->event_body);
				pmadapter->event_body = (t_u8*)mem_calloc((pmadapter->upld_len - MLAN_EVENT_HEADER_LEN) + 32, 1);
				if(!pmadapter->event_body){
					p_err_mem;
					break;
				}
				pmadapter->event_body_len = (pmadapter->upld_len - MLAN_EVENT_HEADER_LEN) + 32;
			}
#endif
...
#if SYS_MONITOR
			if(GET_MONITOR_ITEM_VALUE(event_body_size) < pmadapter->upld_len - MLAN_EVENT_HEADER_LEN){
					   
				SET_MONITOR_ITEM_VALUE(event_body_size,pmadapter->upld_len - MLAN_EVENT_HEADER_LEN);
			}
#endif  
		}
...
t_u32 read_int_status(moal_handle *handle)
{
	t_u32 int_status;
	mlan_status ret;
	pmlan_adapter pmadapter;
	
	pmadapter = (pmlan_adapter)handle->pmlan_adapter;
	ret = pmadapter->callbacks.moal_read_reg(pmadapter->pmoal_handle, HOST_INT_STATUS_REG,
			&int_status);

	if(ret == MLAN_STATUS_SUCCESS)
	{
		if(int_status){
			pmadapter->callbacks.moal_spin_lock(pmadapter->pmoal_handle, pmadapter->pint_lock);
			pmadapter->sdio_ireg |= int_status;
			pmadapter->callbacks.moal_spin_unlock(pmadapter->pmoal_handle, pmadapter->pint_lock);
		}
	}
	return pmadapter->sdio_ireg;
}
