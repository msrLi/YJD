...
int lwip_netif_init()
{
	...
	ipaddr.addr = inet_addr("192.168.1.37");
 ...
 *ip地址范围224.0.0.1 - 239.255.255.255
 */
int udp_add_multicastship(int sock, char *ip)
{
	int err;
	struct ip_mreq imr;
...
/*
 *创建UDP服务器，并且设置为可以接收多播
 *
 *
 */
#define UDP_RECV_BUFFER_SIZE 256
static char udp_buf[UDP_RECV_BUFFER_SIZE + 4];
void udp_server_thread(void *arg)
{
    ...
    //加入多播组
    wifi_set_multicast_only();   //使能硬件多播接收功能
	err = udp_add_multicastship(socket_c, "224.0.0.4");
	
	p_dbg("server ok:%d", err);
	while(1)
	{
		struct sockaddr remote_addr;
		int retval = sizeof(struct sockaddr);
		int size = recvfrom(socket_c, udp_buf, UDP_RECV_BUFFER_SIZE, 0, &remote_addr, (socklen_t*) &retval);
		
		if(size <= 0)
		{
			p_err("udp recv error");
			sleep(10);
			//close(socket_s);	  //Now we dont handle this error
		}
		udp_buf[size]= 0;
		p_err("UDP server get:[%s].\n", udp_buf);
	};
}

void init_udp_server()
{
	thread_create(udp_server_thread, 0, TASK_UDP_SERVER_PRIO, 0, TASK_UDP_SERVER_STK_SIZE, "udp_server_thread");
}
#endif
